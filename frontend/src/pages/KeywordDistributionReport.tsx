    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-12">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 mb-2">Keyword Distribution Report</h1>
                    <p className="text-gray-600">Analysis of search intent across various verticals.</p>
                </div>
            </div>
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2 text-gray-900 border-b border-gray-200 pb-2">
                    ...
                    <h2 className="font-bold text-xl">Shopping Results</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {reportData.Shopping?.map((item, idx) => (
                        <div key={idx} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow group">
                            ...
                            <div className="p-4 space-y-2">
                                <h3 className="font-semibold text-gray-900 line-clamp-2 text-sm h-10" title={item.title}>{item.title}</h3>
                                <div className="flex items-end justify-between">
                                    <span className="font-bold text-lg text-emerald-600">{item.price}</span>
                                    <span className="text-xs text-gray-500 px-2 py-1 bg-gray-100 rounded">{item.source}</span>
                                </div>
                                {item.link && (
                                    <a
                                        href={item.link}
                                        ...
                                        className="mt-3 flex items-center justify-center w-full py-2 bg-indigo-50 text-indigo-600 rounded-lg text-sm font-medium hover:bg-indigo-100 transition-colors"
                                    >
